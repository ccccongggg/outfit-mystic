...
"use strict";
  ...
  // 依赖自检（本文件最先加载，所以放在这里）
  // 脚本改成经典脚本后，顺序由 index.html 决定；排错了会静默拿到 undefined，
  // 页面变成"点了没反应"——那种哑失败最难查。这里让它当场炸，并说清是谁缺了谁。
  NS.require = function (from, deps) {
    const miss = Object.keys(deps || {}).filter((k) => deps[k] === undefined);
    if (miss.length) {
      throw new Error(
        "[Outfit] " + from + " 启动失败：缺少 " + miss.join(" / ") +
        " —— 请检查 index.html 中脚本的加载顺序（vocab → oracle-data → busy → engine → oracle → app → shell）"
      );
    }
  };
...
// ---------- 8 种风格 ----------
const STYLES = ["简约", "通勤", "文艺", "甜美", "运动", "复古", "明艳", "慵懒"];
...
const CATEGORY_NAME = Object.fromEntries(CATEGORIES.map((c) => [c.k, c.name]));

/** 连身单品判定：连衣裙/连体裤自带下装，选中后不再另配 bottom。 */
function isOnepieceType(type) {
 ...
 * 约束写「米」这种宽口径也能命中「米白」。
 * 边角：item 色名为空（脏数据 / 未打标）一律不命中,免得把所有 must_color
 * 都"误命中"而把缺失色藏起来。
 */
function colorHit(itemColorName, keys) {
...
function styleHit(itemTags, keys) {
...
function normalizeItems(items) {
  ...
  // 对外暴露（原来是 export）
  Object.assign(NS, { STYLES, STYLE_ALIAS, COLORS, CATEGORIES, CATEGORY_NAME, isOnepieceType, categoryOf, normColor, normStyle, colorHit, styleHit, normalizeItems });
